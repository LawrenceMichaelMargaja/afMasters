...
const initialState: AppState = {
			...
			? [
					{
						...
						isActive: true,
						organization: "Org A",
						createdBy: "demby.abella",
						createdAt: "2024-01-15",
						lastUpdatedAt: "2024-01-15",
					},
					{
						user: "Lawrence Margaja",
						isActive: true,
						organization: "Org B",
						createdBy: "lawrence.margaja",
						createdAt: "2024-02-20",
						lastUpdatedAt: "2024-01-15",
					},
					{
						user: "Younes Outerbah",
						isActive: true,
						organization: "Org C",
	...
	capturePages:
		localStorage.getItem("dataCaps") === null
			? [
					{
			...
			: JSON.parse(localStorage.getItem("dataCaps") as string),
	checkedItems: [],
};
	...
	reducers: {
		...
		editItem: (
			...
			action: PayloadAction<{
				index: number;
				updatedItem: Organization | CapturePage;
			}>
		) => {
			const { index, updatedItem } = action.payload;
			if (state.dataType === "Organizations") {
				state.organizations[index] = updatedItem as Organization;
				localStorage.setItem("dataOrgs", JSON.stringify(state.organizations));
			} else {
				state.capturePages[index] = updatedItem as CapturePage;
				localStorage.setItem("dataCaps", JSON.stringify(state.capturePages));
			}
		},
	},
});
